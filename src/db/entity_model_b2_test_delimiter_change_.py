...
def store_embeddings_in_chromadb(filtered_entities_df, valid_ids):
    ...
    for index, row in filtered_texts_df.iterrows():
        ...
        for i, chunk in enumerate(chunks):
            ...
            # Convert lists to custom delimiter-separated strings
            custom_delimiter = "^,^"
            entity_texts_str = custom_delimiter.join(entity_texts)
            entity_labels_str = custom_delimiter.join(entity_labels)
...
def query_embeddings(query_text, target_label):
    ...
    for metadata, document in zip(results['metadatas'][0], results['documents'][0]):
        if target_label in metadata['entity_labels'].split(custom_delimiter):  # Adjusting the check for custom delimiter
