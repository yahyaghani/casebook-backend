...
def store_embeddings_in_chromadb(filtered_entities_df, valid_ids):
    ...
    delete_collection(chroma_client, "entity_texts")
    ...
    for index, row in filtered_texts_df.iterrows():
        ...
        for i, chunk in enumerate(chunks):
            ...
            metadata = {
                ...
                "casename_ents": ', '.join([entity_texts[j] for j in range(len(entity_labels)) if entity_labels[j] == 'CASENAME']),
                "citation_ents": ', '.join([entity_texts[j] for j in range(len(entity_labels)) if entity_labels[j] == 'CITATION']),
                "provision_ents": ', '.join([entity_texts[j] for j in range(len(entity_labels)) if entity_labels[j] == 'PROVISION'])
            }

            # print(f'Embedding for: {metadata}', embedding)

            collection.add(
                # documents=[chunk],  # You can choose to remove this if documents are not needed
                metadatas=[metadata],
                ids=[metadata['chunk_id']],
                embeddings=[embedding]
            )
            print(f"Processed global_id: {global_id}")
...
def query_embeddings(query_text, target_label):
    ...
    for metadata in results['metadatas'][0]:
        matched_entities.append({
            ...
            "casename_ents": metadata.get('casename_ents', ''),
            "citation_ents": metadata.get('citation_ents', ''),
            "provision_ents": metadata.get('provision_ents', '')
        })
